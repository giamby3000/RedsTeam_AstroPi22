...
with open('RedsTeamDataLogSamplesNoHaderForMathplot1.csv','r') as csvfile:
	lines = csv.reader(csvfile, delimiter=',')
	for row in lines:
		x.append(row[0])
		y.append((float(row[5])))

plot1.plot(x, y, color = 'r', linestyle = 'solid', marker = '',label = "ISS Elevation profile")

loc = plticker.MultipleLocator(base=750) # this locator puts ticks at regular intervals
plot1.xaxis.set_major_locator(loc)
plot1.legend(loc='lower left')
plot1.set_title('Elevation')
plot1.set_ylabel('Altitude (km)')
plot1.grid(True)
...
fig.suptitle('ASTROPI REDSTEAM ISS ELEVATION ANALYSIS', fontsize=30, color='r')
